...
export type AgentStep = { type: 'invoice_validated' | 'model_call' | 'tool_call' | 'response_generated' | 'error'; tool: string | null; status: 'running' | 'completed' | 'failed'; message: string }
export type AgentError = { code: string; message: string }
export type AgentResponse = { status: 'completed' | 'failed'; message: string; steps: AgentStep[]; tool_results: Array<{ call_id: string; tool: string; status: 'success' | 'error'; output: Record<string, unknown> | null; error: AgentError | null; duration_ms: number }>; claim: Claim | null; audit: AuditResponse | null; error: AgentError | null }
